...
pub fn get_chat_response_large(temp: &str, init: &str, prompt: &str) -> Result<String, reqwest::Error> {
    let base_url = "http://localhost:8008/api/chat";
    let model = "ggml-alpaca-13B-q4_0.bin";
    let temperature = temp;
    let top_k = "50";
    let top_p = "0.95";
    let max_length = "512";
    let context_window = "1536";
    let repeat_last_n = "64";
    let repeat_penalty = "1.3";
    let init_prompt = encode(prompt);
    let n_threads = "2";
    ...
    let question_url = format!("{}/{}/question?prompt={}", base_url, uuid, encode(prompt));
    println!("{}", &question_url);
    Client::new().post(&question_url).timeout(std::time::Duration::from_secs(240)).send()?;
